...
public class TC_MMM_3390 extends KeywordUtil {
	...
	@Test
	public void test_TC_MMM_3390() throws Exception {
		...
		try {
			...
			//Check for the active service provider Tab and add
			...
			executeStep(click(Constants.services.addServiceProviderBtn, Common.type_xpath), getClass(),"Select the servicing provider");
			executionDelay(2000);
			executeStep(click(Constants.Common.next_btn, Common.type_xpath), getClass(),"Click on the next button");
			executeStep(isWebElementPresent(Constants.services.addedServiceProvider, Common.type_xpath), getClass(),"The servicing provider is added to eService Summary");
			
			
			//Check for the Uploaded Document
			executeStep(isWebElementPresent(Constants.services.additionalInfoLink, Common.type_xpath), getClass(),"Additional Information tab is selected");
			executeStep(uploadFilesUsingSendKeys(Constants.services.uploadFile, Constants.Common.type_xpath, getdatafromXls(testCaseID, "uploadFilePath", row)),getClass(), "Select Document to upload ");
			executionDelay(10000);
			executeStep(click(Constants.services.addDocumentbtn, Common.type_xpath), getClass(),"Click on the Add Document button");
			executeStep(selectList(Constants.services.fileType, Constants.Common.type_xpath,getdatafromXls(testCaseID, "Other", row)), getClass(), "Choose File Type");
			executeStep(uploadFilesUsingSendKeys(Constants.services.uploadFile, Constants.Common.type_xpath, getdatafromXls(testCaseID, "uploadFilePath", row)),getClass(), "Select Document to upload ");
			executeStep(click(Constants.services.delUploadedFile, Common.type_xpath), getClass(),"Delete the uploaded File");
